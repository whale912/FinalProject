...
SELECT * FROM emp;
UPDATE emp SET email = 'sooho1400@naver.com' WHERE name = '임진혁';
 SELECT *
 		FROM(
			...
			from pjboard b , emp e
			where b.projectno=3 AND e.empno = b.writer
		START WITH refno = 0
			CONNECT BY PRIOR b.PJBOARDNO = refno
			ORDER siblings BY b.PJBOARDNO DESC )		
			WHERE cnt BETWEEN 1 AND 5
