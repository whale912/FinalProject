...
<body>        
					                                        ...
					                                        <table class="table table-bordered" style="font-size:13px;">
						                                        <tbody>
						                                            ...
						                                            <tr>
						                                                ...
						                                                <td class="text-right">￦<fmt:formatNumber value="${project.budget}"/></td>
						                                            </tr>
						                                        </tbody>
						                                    </table>
					                                    </div>
	...
	<div id="cruTaks" class="gantt_cal_light" title="업무 생성">
		...
		<div style="font-size:10px;margin-left:10px;margin-top:10px;" class="text-danger"><span class="badge-dot badge-danger"></span>&nbsp;는 필수 입력 표시입니다</div>
			<div style="" id="title_label" class="gantt_cal_lsection">
				<label for="title">제목<span style="margin-left:10px" class="badge-dot badge-danger"></span></label>
			</div>
			...
			<div class="gantt_cal_ltext" style="height:22px;">
				<select style="width:100%;" id="memSelect" name="empno">
				</select>
			</div>
			<div id="start_end_date" class="gantt_cal_lsection">
				<label for="startdate">업무 기간<span style="margin-left:10px" class="badge-dot badge-danger"></span></label>
			</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#reg_btn").click(function(){
	    	...
	    	if($("#btn_name").text()=="저장"){
				...
				if($("#cruTaks [name=title]").val()!=""&&
		    		...
		    		$("#cruTaks [name=title]").focus();
		    	}else if($("#cruTaks [name=p_startdate]").val()==""){
		    		alert("업무 시작일자를 입력하셔야 합니다.");
		    		$("#cruTaks [name=p_startdate]").focus();
		    	}else if($("#cruTaks [name=p_enddate]").val()==""){
		    		alert("업무 종료일자를 입력하셔야 합니다.");
		    		$("#cruTaks [name=p_enddate]").focus();
		    	}
	    ...
	    $("#del_btn").click(function(){
	    	if(confirm("업무을 삭제하시겠습니까?")){
	    		var del={};
	    		var taskno = $("#cruTaks [name=taskno]").val();
				...
				gantt.load("${path}/project.do?method=gantt&projectno="+prono,"json"); 
				gantt.refreshData();
				jsonMem();
		...
		var vm = new Vue({
			...
			methods:{
				...
				callJoinable:function(sname,sdeptno){
					this.joinable=[];
					var url = "${path}/project.do?method=joinable&name="+
								sname+"&deptno="+sdeptno;
					var vm = this;
	...
	function callMemOption(){
		var prono="${param.projectno}";
		$.ajax({
			type:"post",
			url:"${path}/project.do?method=mlist&projectno="+prono,
			dataType:"json",
			success:function(data){
				var options = `<option value="미배정">미배정</option>`;
				for(var i=0;i<data.mlist.length;i++){
					options+=`<option value="`+data.mlist[i].empno+`">`+data.mlist[i].name+`</option>`;
				}
				$("#memSelect").html(options);
			},
			error:function(err){
				console.log(err);
			}
