...
(SELECT t.TASKNO id, t.TITLE text, t."TYPE", to_char(t.P_STARTDATE,'DD-MM-YYYY') start_date, 
		...
		AND t.PROJECTNO = 6)
		UNION ALL
		(SELECT t.TASKNO id, t.TITLE text, t."TYPE", to_date(t.P_STARTDATE,'DD-MM-YYYY') start_date, 
		(t.P_ENDDATE-t.P_STARTDATE) duration,
		t.PROGRESS progress, t."OPEN", e.NAME "users", 
	...
	SELECT * FROM emp WHERE name='김새로이';
...
SELECT  p.projectno, p.pname, k.kname kname, e.name name, p.personnel,
		...
		FROM project p, kind k, emp e
		WHERE p.empno = e.empno AND p.kindno=k.kindno AND p.empno=201910001
		ORDER BY CASE WHEN state='진행' THEN 0 END , startdate DESC; 
	
SELECT ROUND(COUNT(CASE WHEN STATE='승인완료' THEN 1 END)/COUNT(*)) 
FROM task t, 
(SELECT p.PROJECTNO 
...
UPDATE PROJECT SET PROGRESS = 0.9 WHERE PROJECTNO = 6;
