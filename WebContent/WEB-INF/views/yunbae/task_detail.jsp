    ...
    <div class="dashboard-main-wrapper">
        ...
        <div id="wholeContainer" class="dashboard-wrapper">
            <div class="dashboard-ecommerce">
                <div id="mainContainer" class="container-fluid dashboard-content" style="min-height:844px;">
                    <!-- ============================================================== -->
                    <!-- pageheader  -->
                    <!-- ============================================================== -->
                    <div class="row">
                    ...
                    <div class="ecommerce-widget">
                    	...
                    	<div class="row">
                    		<div id="divisionTb" class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
		                        <div class="card">
    ...
    <div id="upload_result" title="결과 업로드">
    	<div class="form-group">
    		<form id="uploadFrm" enctype="multipart/form-data" method="post">
				...
				<input style="width:90%;float:left;text-align:right;font-size:13px;" name="progress" type="text" class="form-control" placeholder="진행률입력 (0~100, 10단위)%"/><span style="float:left;width:10%;text-align:center;font-size:13px;height:38px;line-height:38px">%</span><br>
...
<script type="text/javascript">
	...
	function uploadTaskD(divno, progress){
		...
		$( "#upload_result" ).dialog({
		        ...
		        buttons:{
		        	"업로드" : function(){
		        	 	if($("#upload_result [name=progress]").val()!=""&&
		        	 			$("#upload_result [name=progress]").val()*1>0&&
		        	 			$("#upload_result [name=progress]").val()*1<=100){
			        		...
			        		var data = new FormData(form);
		        	 	...
		        	 	}else if($("#upload_result [name=progress]").val()==""){
		        	 		alert("진행률을 입력해주세요!");
		        	 		$("#upload_result [name=progress]").val("");
		        	 		$("#upload_result [name=progress]").focus();
		        	 	}else if($("#upload_result [name=progress]").val()*1<0){
		        	 		alert("진행률은 음수일 수 없습니다");
		        	 		$("#upload_result [name=progress]").val("");
		        	 		$("#upload_result [name=progress]").focus();
		        	 	}else if($("#upload_result [name=progress]").val()*1==0){
		        	 		alert("진행률이 0%입니다");
		        	 		$("#upload_result [name=progress]").val("");
		        	 		$("#upload_result [name=progress]").focus();
		        	 	}else if($("#upload_result [name=progress]").val()*1>100){
		        	 		alert("진행률은 100%보다 클 수 없습니다");
		        	 		$("#upload_result [name=progress]").val("");
		        	 		$("#upload_result [name=progress]").focus();
		        	 	}else if(isNaN($("#upload_result [name=progress]").val()*1)){
		        	 		alert("진행률에는 숫자만 입력해주세요 !");
		        	 		$("#upload_result [name=progress]").val("");
		        	 		$("#upload_result [name=progress]").focus();
		        	 	}
		        	},
